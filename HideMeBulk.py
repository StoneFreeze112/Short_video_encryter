...
# Just run with python Hideme_BULK_WORKING.py   → it works no matter how you paste
...
# ========================== RECOVER ONE ==========================
def recover(enc_file: Path, password):
    try:
        ...
        print("\nWrong password or bad file!")

# ========================== MAIN ==========================
while True:
    clear()
    print("=" * 70)
    print("          HIDEME – BULK ENCRYPTION (NOW REALLY WORKS)")
    print("=" * 70)
    print("(b) Bulk encrypt")
    print("(r) Recover one .enc file")
    print("(q) Quit")
    choice = input("\nChoose → ").strip().lower()
    ...
    # THIS IS THE FIX → keep quotes and spaces exactly as you pasted
    import shlex
    try:
    ...
    if choice == 'b':
        ...
        recover(valid_files[0], password)
